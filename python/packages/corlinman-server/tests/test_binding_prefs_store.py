...
def test_absent_row_returns_defaults(tmp_path: Path) -> None:
    ...
    assert prefs.persona_id is None
...
def test_set_clear_persona_id_preserves_model_and_epoch(tmp_path: Path) -> None:
    db = tmp_path / "prefs.sqlite"
    set_model_override(*_B, "alias-x", db_path=db)
    bump_session_epoch(*_B, db_path=db)

    prefs = set_persona_id(*_B, "alice", db_path=db)
    assert prefs.model_override == "alias-x"
    assert prefs.persona_id == "alice"
    assert prefs.session_epoch == 1

    cleared = set_persona_id(*_B, None, db_path=db)
    assert cleared.persona_id is None
    assert cleared.model_override == "alias-x"
    assert cleared.session_epoch == 1


def test_epoch_bumps_monotonically_and_keeps_model_and_persona(
    tmp_path: Path,
) -> None:
    db = tmp_path / "prefs.sqlite"
    set_model_override(*_B, "alias-x", db_path=db)
    set_persona_id(*_B, "alice", db_path=db)
...
def test_bindings_are_isolated(tmp_path: Path) -> None:
    ...
    assert get_prefs(*other, db_path=db).persona_id is None
    assert get_prefs(*_B, db_path=db).session_epoch == 0


def test_existing_db_without_persona_column_is_migrated(tmp_path: Path) -> None:
    db = tmp_path / "prefs.sqlite"
    with sqlite3.connect(db) as conn:
        conn.executescript(
            """
            CREATE TABLE binding_prefs (
            ...
            VALUES
                ('qq|bot1|group42|user7', 'qq', 'bot1', 'group42', 'user7',
                 'alias-x', 3, 1);
            """
        )
