...
_PERSONA_WIZARD_PRELUDE: str = (
    ...
    "    Stage 0a — 用 ask_user 收集角色名 + 可选一手资料（粘贴文本或 URL）。\n"
    "    Stage 0b — 调研：用户给了本地资料就先用本地；否则 **MUST call "
    "`web_search` 至少 1 次** 再 `web_fetch` 2-3 个权威结果。**绝对禁止凭"
    "训练语料填 bucket** —— 抓取失败 ⚠️ 显式标注，让用户选 `重做` 或 `改自创`。\n"
