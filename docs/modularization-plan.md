...
- **What moves:** `_write_config_atomic` (single definition in `routes_admin_b/onboard.py`, imported by `credentials.py` / `providers.py`) into one neutral `gateway/core/config_mutation.py` leaf; the config modules import from there instead of from `onboard`. **Phase 1 ✅ PR #55 (merged).** Note: the two `_redact()` functions are *different* (config dict-redactor vs message-scrubber) and were deliberately **not** merged.
...
Relocates `_write_config_atomic` → `gateway/core/config_mutation.py` (`write_config_atomic`); repoints `onboard`/`credentials`/`providers`, keeps a re-export shim, adds unit tests. Scope-corrected against the live tree (single def, not 4 copies; the two `_redact()` left untouched). Future config helpers (snapshot, sub-key merge) can join this leaf as they're touched.
...
| Config-write seam location | `_write_config_atomic` defined in a route module (`onboard`) | relocated to `gateway/core/config_mutation` leaf | Phase 1 / PR #55 (merged) |
